...
String inString = ""; // holds inp[uts from file
...
void updateHeartRate(){ // interrupt handler
    ...
    if (Serial.available()){
      ...
      while(Serial.available()){
        int inChar = Serial.read(); // read the incoming data
      ...
      if(QRS_detected == true){
        ...
        queue.enqueue(bpm/BPM_BUFFER_SIZE); // add the latest BPM to the queue to be printed
